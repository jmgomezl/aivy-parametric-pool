...
export function demoStore(directory=path.join(process.cwd(),'.artifacts')){
 ...
 return {read,save,
  ...
  begin(id,requestId,kind,amount,now=Date.now()){
   if(!/^[a-zA-Z0-9-]{16,80}$/.test(requestId)||!['deposit','cover'].includes(kind)||!Number.isFinite(amount)||amount<=0||amount>100)throw Object.assign(Error('Invalid demo action.'),{status:400});
