...
export async function deposit(client, { tokenId, treasuryId, poolId, lpId, lpKey, amountUnits, network, checkpoint = () => {} }) {
  ...
  const mint = await new TokenMintTransaction()
    .setTokenId(tokenId).setAmount(units).freezeWith(client);
  await checkpoint({mintTxId:mint.transactionId.toString()});
  const minted=await mint.execute(client);await minted.getReceipt(client);
  ...
  await checkpoint({depositTxId:signed.transactionId.toString()});
