...
add('map',3,await clip('map','atlas',6));
...
add('guard',10,await clip('story','guard',7));
add('crosschain',7);
...
add('companion',10,{phase:'quorum-companion',start:0,duration:10},{mediaCrop:'390:700:1510:284',rect:[1110,140,490,880]});
...
for(let i=0;i<shots.length;i++){
 ...
 if(shot.clip){
  const source=path.join(root,'assets/footage',shot.clip.phase==='quorum-companion'?'quorum-companion.mp4':`${id}-${shot.clip.phase}.mp4`);
