...
try{
 for(const width of [1600,1024,768,390,320]){
  ...
  for(const [index,start] of [[3,44],[4,67],[7,121],[10,177],[11,201],[12,215],[13,231]]){
  ...
  await p.locator('#aivy-preview').click();await p.waitForTimeout(400);assert.equal(await p.locator('#cue-count').innerText(),'11 / 14');assert.equal(await p.locator('video').evaluate(v=>v.paused),false);await p.locator('video').evaluate(v=>v.pause());
  ...
  assert.deepEqual(errors,[]);assert.deepEqual(failed,[]);report.push({width,duration:239,videoWidth:1920,chapters:true,playback:true,noOverflow:true,errors,failed});await ctx.close();console.log('Studio passed',width);
 }
