...
Maintained in its own [Take Studio repository](https://github.com/jmgomezl/aivy-take-studio),
with an independent static deployment. It is not part of the Quorum product.
...
remain economic previews for one complete policy term; annual premium comparisons
are secondary and are not expected returns. **A deposit cannot select a policy.**
[Funding model, worked example and verified numbers](docs/ECONOMIC-MODEL.md).
...
**Verified live:** [three settled oracle payments, totaling 0.003 test aUSDd](docs/evidence/blocky402-testnet.json),
with Blocky sponsoring the network fees. Open **Onchain → x402** to inspect them.
...
| Capacity and replay protection | Exclusive issuance lock, fresh same-asset pool balance and durable reservation before ledger writes. HBAR and each token have separate exposure totals; unknown assets fail closed. Reusing a request ID does not mint again. Interrupted writes retain reservations and receipt checkpoints. A permanent kernel lock serializes recovery across processes; dead-owner markers are reclaimed inside it, while malformed markers refuse work. | [Issuance](src/policy/issue.js), [asset accounting](src/pool/exposure.js), [book](src/book.js), [lock](src/issuance-lock.js), [safety tests](tests/safety.test.js) |
