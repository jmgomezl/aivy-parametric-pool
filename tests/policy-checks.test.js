...
test('a confirmed payment survives catalogue failure without claiming a negative vote or signature',async()=>{
 const f=fixture();try{
  const pay=async(url,o)=>{const tx='0.0.1@123.000000001';await o.checkpoint({transactionId:tx});return {paid:true,response:{ok:true,json:async()=>({sourceKey:new URL(url).hostname.split('.')[0],triggered:false,unavailable:true,verdict:'Catalogue unavailable.',payment:{transaction:tx,facilitator:BLOCKY_INFO},signature:{signed:true,transactionId:'untrusted'}})}};};
...
test('an explicit source-bound preflight refusal leaves no payment to repeat or reconcile',async()=>{
 const f=fixture();try{
  const pay=async(url,o)=>{await o.checkpoint({transactionId:'0.0.7162784@123.000000001'});return {paid:false,response:{ok:false,status:503,json:async()=>({error:'blocky_unavailable',paymentSubmitted:false,sourceKey:new URL(url).hostname.split('.')[0]})}};};
  const r=await checkPolicy({...f.config,pay});assert.equal(r.cost,0);assert.equal(r.needsReview,false);assert.ok(r.checks.every(c=>c.status==='unavailable'&&!c.paymentTxId));
 }finally{f.done();}
});
