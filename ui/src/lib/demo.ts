...
export async function demoCall(path:string,body?:unknown){const response=await fetch(BASE+path,{method:body===undefined?'GET':'POST',headers:{'content-type':'application/json',authorization:`Bearer ${ensureToken()}`},...(body===undefined?{}:{body:JSON.stringify(body)}),signal:AbortSignal.timeout(120000)});const r=await response.json();if(!response.ok||r.ok===false)throw Object.assign(Error(r.message??'Demo request unavailable.'),{status:response.status});return r;}
