...
function drawEvents(ctx: CanvasRenderingContext2D, v: View, minMag: number, fromDay: number, toDay: number) {
  ...
  const alpha = Math.min(0.4, 0.035 / Math.sqrt(v.k)); // brighter fields at zoom would saturate to white
  ...
  for (const q of QUAKES) {
    ...
    const big = q.mag >= 7.5;
    ctx.fillStyle = big ? 'rgba(227,179,65,0.65)' : 'rgba(227,179,65,0.22)';
