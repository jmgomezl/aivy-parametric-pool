...
export function PositionNFT({policy:p,kind='cover',contribution=100,compact=false}:Props){
 ...
 return <article className={`position-nft ${lp?'position-nft-lp':''} ${compact?'position-nft-compact':''}`} aria-label={`${lp?'Liquidity position preview':'Cover NFT'} for ${name}, ${number(amount)} ${asset}`}>
   ...
   <header className="nft-header"><span className="nft-brand"><QuorumMark/> AIVY QUORUM</span><span>{lp?'FUNDING ESTIMATE':'COVER NFT'} <span className="num">/{String(p.serial).padStart(4,'0')}</span></span></header>
   ...
   <div className="nft-value"><span>{lp?'Example contribution':state==='paid'?'Payout executed':'Scheduled payout'}</span><div><strong className="num">{number(amount)}</strong><span>{asset}</span></div></div>
   <div className="nft-position-line"><span style={{width:lp?`${percentage}%`:state==='paid'?'100%':`${p.ledger?.available?signed/2*100:0}%`}}/></div>
   <footer className="nft-footer"><span className="nft-status"><i/>{lp?`${number(percentage)}% of policy target`:statusLabel(p)}</span><span>{lp?'PREVIEW ONLY':p.network??'testnet'}</span></footer>
 </article>;
}
