...
export function ChainActivity({ serial, story = false, swap = false }: { serial?: string; story?: boolean; swap?: boolean }) {
  ...
  useEffect(() => { setSelected(''); setTab(story ? 'mainnet' : swap ? 'bridge' : 'live'); }, [serial, story, swap]);
  useEffect(() => {
  ...
  return <section className="chain-activity" aria-label="Blockchain activity" onKeyDown={event=>{if(open&&event.key==='Escape'){event.preventDefault();event.stopPropagation();setOpen(false);toggle.current?.focus();}}}>
    ...
    {open ? <div id={id} className="chain-panel">
      ...
      {tab === 'live' ? <>
      ...
      </> : tab==='bridge'?<SwapEvidence/> : <>
        <div className="chain-context"><p><strong>Testnet</strong> · Blocky402 facilitator</p></div>
        <div className="chain-payment-flow" aria-label="x402 payment flow"><span>402 request</span><i>→</i><span>Blocky402 payment</span><i>→</i><span>Oracle response</span></div>
        <p className="chain-note">HTTP requests and payment checks run offchain. Blocky402 settles on Hedera before the oracle serves the result.</p>
        ...
        {payments.length ? <div className="chain-receipts">{payments.map(r=><Receipt key={r.transaction} network={r.network} proof={{label:r.asset==='0.0.10374011'&&a.pool?.asset.tokenId===r.asset?`Oracle payment · ${(Number(r.amount)/1e6).toLocaleString(undefined,{maximumFractionDigits:6})} ${a.pool.asset.symbol}`:'Oracle payment settled',state:r.facilitator?.name==='Blocky402'?'Blocky402 · settled':'Earlier x402 · settled',href:hashscan('transaction',r.transaction,r.network),why:`${r.amount} base units of ${r.asset} for ${r.resource}. Recorded ${new Date(r.at).toLocaleString()}. A payment receipt alone does not prove that an oracle approved a payout.`}}/>)}</div> : paymentState !== 'error' ? <p className="chain-note">{paymentState==='loading'?'Reading payment receipts…':'No receipts recorded since activity tracking was enabled.'}</p> : null}
        <details className="chain-mainnet-x402"><summary>How Blocky402 is used</summary><p><a href="https://api.testnet.blocky402.com/supported" target="_blank" rel="noreferrer">Hosted Blocky402 · Hedera testnet ↗</a> verifies and co-signs each payment, sponsoring its gas. Quorum validates the signed terms before sending them. Older receipts retain their original facilitator; no mainnet Blocky payment is claimed.</p></details>
