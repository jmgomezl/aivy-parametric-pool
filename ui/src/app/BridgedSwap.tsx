...
export function BridgedSwap(){
 ...
 return <details className="testnet-swap"><summary>Swap bridged aUSDd <span>Uniswap ↗</span></summary><div className="testnet-swap-body"><div className="conversion-top"><span>Sepolia · test tokens only</span><span>0.5% slippage</span></div><h4>aUSDd → test USDC</h4><p>A real Uniswap pool with sponsored test liquidity. Prices can move; this is not a USD peg or cash redemption.</p><label>Spend bridged aUSDd<input value={amount} inputMode="decimal" disabled={busy||Boolean(pending)} onChange={e=>{setAmount(e.target.value);setDraft(null);setApproved(false);}}/></label><button className="chip" disabled={busy||Boolean(pending)} onClick={()=>void run(quote)}>Connect wallet & get quote →</button>
