...
export function PoliciesPage(){
  ...
  const rows=[...(a.policies??[])].filter(p=>!funding||p.asset==='aUSDd').filter(p=>filter==='global'&&featured.has(p.serial)||filter==='all'||filter==='recent'||filter==='here'&&created.has(String(p.serial))||filter==='active'&&['active','confirming'].includes(policyState(p))).sort((x,y)=>Number(y.serial)-Number(x.serial)).slice(0,filter==='recent'?6:undefined);
  return <div className="page"><div className="page-inner policies-page">
    <div className="page-title-row"><div><div className="eyebrow">{a.network} demo</div><h1>{funding?'Fund earthquake cover':'Policies'}</h1><p>{funding?'Deposit into the shared pool. Explore the policies it backs.':'View cover or explore its funding side.'}</p></div><a href="/" onClick={onLink} className={funding?'chip':'buy compact'}>Create demo cover <span>↗</span></a></div>
    <div className="position-tabs policy-view-tabs" aria-label="Policy view"><button aria-pressed={!funding} onClick={()=>setView(false)}>Cover receipts</button><button aria-pressed={funding} onClick={()=>setView(true)}>Fund cover</button></div>
    {funding?<><FundPool/><h2 className="funding-explore-heading">Explore policy economics</h2><p className="funding-note">Compare hypothetical policy economics. Every real deposit backs the shared pool.</p></>:null}
    <div className="policy-filters" aria-label="Filter policies">{[['global','Global demos'],['recent','Recent'],['here','Created here'],['all','All policies'],['active','Active cover']].map(([id,label])=><button key={id} className={`chip ${filter===id?'chip-on':''}`} aria-pressed={filter===id} onClick={()=>{setFilter(id);updateQuery(funding,id);}}>{label}</button>)}<button className="text-button" onClick={()=>void refresh()}>Refresh ↻</button></div>
