...
export function PoliciesPage(){
  const a=useAgent(), [filter,setFilter]=useState('global');
  const [funding,setFunding]=useState(()=>new URLSearchParams(location.search).get('view')==='fund');
  const setView=(value:boolean)=>{setFunding(value);history.replaceState(null,'',value?'/policies?view=fund':'/policies');};
  useEffect(()=>{const sync=()=>setFunding(new URLSearchParams(location.search).get('view')==='fund');window.addEventListener('popstate',sync);return()=>window.removeEventListener('popstate',sync);},[]);
  ...
  return <div className="page"><div className="page-inner policies-page">
    <div className="page-title-row"><div><div className="eyebrow">{a.network} demo</div><h1>{funding?'Fund a policy':'Policies'}</h1><p>{funding?'Choose a place. Back its cover. Share its premiums.':'View cover or explore its funding side.'}</p></div><a href="/" onClick={onLink} className="buy compact">Create demo cover <span>↗</span></a></div>
    ...
    <div className="policy-filters" aria-label="Filter policies">{[['global','Global demos'],['recent','Recent'],['here','Created here'],['all','All policies'],['active','Committed']].map(([id,label])=><button key={id} className={`chip ${filter===id?'chip-on':''}`} aria-pressed={filter===id} onClick={()=>setFilter(id)}>{label}</button>)}<button className="text-button" onClick={()=>void refresh()}>Refresh ↻</button></div>
