...
export function PayoutConversion({ usd }: { usd: number }) {
  ...
  return <details className="payout-conversion" open={open} onToggle={e => setOpen(e.currentTarget.open)}>
    ...
    {open ? <div className="conversion-body">
      <div className="conversion-purpose"><strong>Hedera cover. Uniswap liquidity quotes.</strong><span>Agent API integration · no custom Solidity contracts</span></div><div className="conversion-top"><span>Mainnet · quote only</span><label><span className="sr-only">Quote network</span><select aria-label="Quote network" value={chain} onChange={e => setChain(Number(e.target.value))}><option value={8453}>Base</option><option value={130}>Unichain</option></select></label></div>
      ...
      <p className="conversion-boundary">This mainnet quote does not move funds. Try a separate testnet swap below.</p>
