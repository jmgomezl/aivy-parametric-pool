...
export function LPPreviewControls({policy,portion,onPortion}:{policy:Policy;portion:number;onPortion:(value:number)=>void}){
 ...
 return <div className="lp-preview-controls">
   <div className="lp-preview-heading"><h3>Funding {placeName(policy).split(',')[0]}</h3><span>Preview</span></div>
   ...
   {model?<><div className="lp-income"><div><span>Estimated yearly income rate</span><strong className="num">{number(model.annualRate)}%<small> / yr</small></strong></div><div><span>Premiums · {model.horizonDays} days</span><strong className="num">+{number(model.income)}</strong><small>{asset}</small></div></div>
