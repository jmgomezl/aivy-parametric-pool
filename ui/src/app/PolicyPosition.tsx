...
export function LPPreviewControls({policy,portion,onPortion}:{policy:Policy;portion:number;onPortion:(value:number)=>void}){
 ...
 return <div className="lp-preview-controls">
   ...
   {model?<><div className="lp-income"><div><span>Annual premium rate</span><strong className="num">{number(model.annualRate)}%<small> / yr</small></strong></div><div><span>Premiums · {model.horizonDays} days</span><strong className="num">+{number(model.income)}</strong><small>{asset}</small></div></div>
   ...
   <div className="lp-preview-disclosure">Per-policy estimate · shared-pool deposits available above · {asset} has no cash value.</div>
 </div>;
}
