...
export function LPPreviewControls({policy,portion,onPortion}:{policy:Policy;portion:number;onPortion:(value:number)=>void}){
 ...
 return <div className="lp-preview-controls">
   ...
   <details className="lp-assumptions"><summary>Model & limitations <span>+</span></summary><p>{Math.round(model.poolFraction*100)}% of this policy’s premium goes to the pool{policy.brokerId?' after the 15% broker split':''}. The preview allocates it in proportion to each contribution. Annual rate = pool premium ÷ payout target × 365 ÷ {model.days} days. Assumes continuous funding at the same terms, without compounding. Claims, fees and idle capital reduce returns; losses can consume your entire contribution.</p><p>Longer periods assume renewal at the same rate, without compounding. A first-term claim consumes your capital and stops further income. Timing and costs change actual outcomes.</p><p>No per-policy deposit, income distribution or LP NFT is created here. Current ARPS deposits back the shared pool; withdrawals and income distributions are not enabled.</p></details></>:<p>Return estimate unavailable for this policy’s recorded terms.</p>}
