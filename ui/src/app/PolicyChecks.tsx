...
export function PolicyChecks({serial,enabled,legacy=false}:{serial:string;enabled:boolean;legacy?:boolean}){
 ...
 return <section className="policy-checks" aria-label="Earthquake event checks"><div className="capital-heading"><strong>Check for earthquakes</strong><span>Testnet</span></div><p>Three catalogues check the recorded terms. Matching events can trigger the payout.</p>{legacy?<p className="check-legacy" role="note">This policy was issued before the current terms format. Its record on HCS predates the fields the oracles verify, so no catalogue will attest it. Buy a policy to run a live check.</p>:<><button className="chip" disabled={busy||!enabled||check?.status==='pending'||check?.needsReview} onClick={()=>void run()}>{busy?'Checking sources…':pending?'Check original request ↻':account?'Check now · up to 0.003 aUSDd →':'Start demo account to check →'}</button><small>x402 · 0.001 per source · five-minute shared cooldown</small></>}
