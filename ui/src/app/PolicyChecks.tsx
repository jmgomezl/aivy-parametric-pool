...
export function PolicyChecks({serial,enabled}:{serial:string;enabled:boolean}){
 ...
 return <section className="policy-checks" aria-label="Earthquake event checks"><div className="capital-heading"><strong>Check earthquake conditions</strong><span>Testnet</span></div><p>Three sources check this policy’s recorded terms. Qualifying signatures can release its scheduled payout.</p><button className="chip" disabled={busy||!enabled||check?.status==='pending'||check?.needsReview} onClick={()=>void run()}>{busy?'Checking sources…':pending?'Check original request ↻':account?'Check now · up to 0.003 aUSDd →':'Start demo account to check →'}</button><small>x402 · 0.001 per source · five-minute shared cooldown</small>
 {check?<div className="policy-check-results">{check.checks.map(c=><div key={c.source}><div><strong>{c.source}</strong><span className={c.status==='no-match'?'text-fg-2':c.status==='qualifying-event'?'text-ok':'text-pending'}>{c.status==='no-match'?'No qualifying event':c.status==='qualifying-event'?'Qualifying event':c.status==='checking'?'Checking…':c.status==='unavailable'?'Unavailable':'Receipt needs review'}</span></div><details><summary>Evidence & receipt</summary><p>{c.verdict}</p>{c.paymentTxId?<a href={receipt(c.paymentTxId)} target="_blank" rel="noreferrer">x402 payment {c.paid?'confirmed':'submitted'} ↗</a>:null}{c.signatureTxId?<a href={receipt(c.signatureTxId)} target="_blank" rel="noreferrer">Oracle signature ↗</a>:null}</details></div>)}{check.checkedAt?<small>Checked {new Date(check.checkedAt).toLocaleString()} · ledger signatures shown above</small>:null}</div>:null}{message?<p role="status">{message}</p>:null}
 </section>;
}
